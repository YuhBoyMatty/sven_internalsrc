...
static usermsg_t *g_pClientUserMsgs;

//--------------------------------------------------------------------------------------------------------------

pfnUserMsgHook HookUserMsg(const char *pszUserMessage, pfnUserMsgHook pfnHook)
{
	usermsg_t *pUserMsg = g_pClientUserMsgs;

	while (pUserMsg)
	{
		if (!strcmp(pszUserMessage, pUserMsg->szMsg))
		{
			pfnUserMsgHook pfnOriginalHook = pUserMsg->pfnHook;
			pUserMsg->pfnHook = pfnHook;

			return pfnOriginalHook;
		}

		pUserMsg = pUserMsg->pNext;
	}
...
void InitUserMsg()
{
	...
	BYTE *pHookUserMsg = (BYTE * )g_pEngineFuncs->pfnHookUserMsg;
	pHookUserMsg += 0x8;

	get_instruction(&instruction, pHookUserMsg, MODE_32);

	if (instruction.type == INSTRUCTION_TYPE_CALL && instruction.op1.type == OPERAND_TYPE_IMMEDIATE)
	{
		// Call address + relative address + pointer size + CALL opcode
		pHookUserMsg = (BYTE *)((DWORD)pHookUserMsg + (DWORD)instruction.op1.immediate + sizeof(void *) + 1);
	}
	else
	{
	...
	{
		g_pClientUserMsgs = *reinterpret_cast<usermsg_t **>(instruction.op2.displacement);
	}
	else
	{
